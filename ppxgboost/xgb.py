...
class BinaryClassificationLoss():
    ...
    def compute_residual(self, dataset, subset, predicted):
        ...
        h = dict()
        for id in subset:
            g[id] = - exp(-dataset.get_instance(id)['diagnosis'] * predicted[id]) * dataset.get_instance(id)['diagnosis']
            h[id] = exp(-dataset.get_instance(id)['diagnosis'] * predicted[id]) * (dataset.get_instance(id)['diagnosis']**2)
        return g, h
...
class RegressionLossFunction():
    def compute_residual(self, dataset, subset, predicted):
        ...
        h = dict()
        for id in subset:
            g[id] = -2 * (dataset.get_instance(id)['diagnosis'] - predicted[id])
            h[id] = 2.0
        return g, h
    ...
    def fit(self, dataset, train_data, trees, e, delta):
        if self.loss_type == 'multi-classification':
            ...
            if len(trees) >= 1:
                for it in range(1, len(trees) + 1):
                    for lab in label_valueset:
                        for i in subset:
                            predicted[lab][i] += self.learn_rate * trees[it][lab].get_predict_value(
                                dataset.get_instance(i))
            ...
            for label in label_valueset:
                ...
                leaf_nodes = []
                tree = construct_decision_tree(dataset, subset, predicted, e, delta, leaf_nodes,
                                               0, self.max_depth, g, h, self.split_points)
                trees[label] = tree
            return trees
        else:
            ...
            g, h = self.loss.compute_residual(dataset, subset, predicted)
            m = 0.8
            p = abs(sum(g) / (sum(h) + m))
            leaf_nodes = []
            tree = construct_decision_tree(dataset, subset, predicted, e, delta, leaf_nodes,
                                           0, self.max_depth, g, h, m, p, self.split_points)
            return tree
